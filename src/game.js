...
import { saveEntry, getEntries } from '@/features/firebase/firestore';
import { set, get, update as rtdbUpdate, onValue, remove, onDisconnect } from '@/features/firebase/rtdb';
...
export function saveGuestName(val) {
    const trimmed = val.trim();
    if (trimmed)
        localStorage.setItem('pong_guest_name', trimmed);
    else
        localStorage.removeItem('pong_guest_name');
}
export function showNameSaved() {
    const el = document.getElementById('name-saved');
    el.style.opacity = '1';
...
export function showMenu() { sfx.click(); showScreen('menu'); }
export async function showLeaderboard() { sfx.click(); showScreen('leaderboard'); await renderLeaderboard(); }
// ── Responsive ─────────────────────────────────────────────────────────────────
...
export function selectMode(m) {
    sfx.click();
    mode = m;
    const btnCpu = document.getElementById('btn-vs-cpu');
    const btnHuman = document.getElementById('btn-vs-human');
...
export function selectDiff(d) {
    sfx.click();
    difficulty = d;
    ['easy', 'medium', 'hard'].forEach(x => document.getElementById('btn-' + x).classList.toggle('active', x === d));
}
// ── Online lobby UI helpers ────────────────────────────────────────────────────
export function showOlChoice() {
    ;
...
export function showJoinForm() {
    ...
    const joinForm = document.getElementById('ol-join-form');
...
function showOlWaiting(code) {
    ...
    document.getElementById('ol-choice').style.display = 'none';
    document.getElementById('ol-join-form').style.display = 'none';
    const waiting = document.getElementById('ol-waiting');
...
export async function joinRoom() {
    ...
    myName = myDisplayName();
    opponentName = hostData?.name || 'Host';
    onlineCode = code;
    onlineRole = 'guest';
    roomRef = ref;
    ...
    if (roomRef) {
        ...
        if (rtdb && onlineCode) {
            if (onlineRole === 'host')
                ...
                remove(dbRef(rtdb, `rooms/${onlineCode}/guest`));
        }
        roomRef = null;
    }
    onlineRole = null;
...
function setupRTDBListeners() {
    ...
    if (onlineRole === 'host') {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        sub('guest/paddleY', (snap) => {
            if (snap.val() !== null)
                state.right.y = snap.val();
        });
        // Host watches for guest leaving
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        sub('guest', (snap) => {
            if (running && snap.val() === null)
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        sub('host/paddleY', (snap) => {
            if (snap.val() !== null)
                state.left.y = snap.val();
        });
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        sub('score', (snap) => {
            const s = snap.val();
            if (s) {
                state.left.score = s.host || 0;
                state.right.score = s.guest || 0;
            }
        });
        ...
        function cdFrame() {
            ...
            if (onlineRole === 'host') {
                if (tp.left !== null)
                    ...
                    if (keys['w'] || keys['W'])
                        state.left.y = clamp(state.left.y - SPD, 0, H - PAD_H);
                    if (keys['s'] || keys['S'])
                        state.left.y = clamp(state.left.y + SPD, 0, H - PAD_H);
                }
                syncHostToRTDB();
                ...
                if (tp.right !== null)
                    ...
                    if (keys['ArrowUp'])
                        state.right.y = clamp(state.right.y - SPD, 0, H - PAD_H);
                    if (keys['ArrowDown'])
                        state.right.y = clamp(state.right.y + SPD, 0, H - PAD_H);
                }
                syncGuestToRTDB();
...
function checkOnlineWin() {
    if (state.left.score < WIN_SCORE && state.right.score < WIN_SCORE)
        return false;
    running = false;
...
function updateAI() {
    ...
    if (center < state.aiTarget - 4)
        state.right.y += spd;
    else if (center > state.aiTarget + 4)
        state.right.y -= spd;
...
function loop() { update(); draw(); if (running)
...
async function saveResult(winner, displayName, modeLabel) {
    ...
    try {
        await saveEntry(entry, myDisplayName());
    }
    catch (e) {
